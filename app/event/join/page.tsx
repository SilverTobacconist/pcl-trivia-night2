...
export default function EventJoinPage() {
  ...
  const [name, setName] = useState(""); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [needsController, setNeedsController] = useState(false); const [isPaused, setIsPaused] = useState(false); const [decline, setDecline] = useState(false);
  useEffect(() => { if (!game) return; fetch(`/api/session-by-code?sessionCode=${encodeURIComponent(game)}`).then((response) => response.json()).then((data) => { setNeedsController(Boolean(data.needsController)); setIsPaused(Boolean(data.isPaused)); }); }, [game]);
  ...
  return <main className="event-shell roper-trivia"><p className="roper-trivia-kicker">Paul&apos;s Cigar Lounge presents</p><h1>Join Roper Trivia</h1><p>Game #{game || "—"}</p><form className="event-card" onSubmit={join}>{isPaused ? <><h2>Game paused</h2><p>Joining this paused game makes you the Decision Player.  After you join, tap Resume to continue exactly where it left off.</p></> : needsController && <><h2>You’re first in line</h2><p>You can be the Decision Player: start the game, skip a completed countdown, and pass control if needed.</p><label><input type="checkbox" checked={decline} onChange={(event) => setDecline(event.target.checked)} /> Let the next player be the controller instead</label></>}<label>Your display name<input value={name} onChange={(event) => setName(event.target.value)} maxLength={40} required autoFocus /></label><button disabled={busy}>{busy ? "Joining…" : "Join game"}</button></form>{error && <p className="event-error">{error}</p>}</main>;
}
