...
export default function EventPage() {
  ...
  return <main className="event-shell"><h1>PCL Trivia Night</h1><h2>Bartender Console</h2>{!session ? <section className="event-card"><button onClick={createEvent}>Start Trivia Night</button><a className="event-secondary-link" href="/event-trivia">Start Event Trivia</a><p>Trivia Night creates the QR-code game for players.  Event Trivia is the separate keyword-driven question display for special events.</p></section> : <section className="event-card"><h2>Game #{session.session_code}</h2><p>Have the first player scan this QR code. They choose the game path and the game runs itself.</p>{qr && <img src={qr} alt="QR code to join trivia" style={{ maxWidth: "100%" }} />}<p><a href={`/event/display?game=${session.session_code}`} target="_blank">Open display screen</a></p></section>}<section className="event-card"><h2>Leaderboards</h2><p>View every current and past game, newest first.</p><a className="event-secondary-link" href="/history">View all leaderboards</a></section>{error && <p className="event-error">{error}</p>}</main>;
}
