...
export async function GET(request: Request) {
  ...
  const rickhouse = rickhouseGame ? { game: rickhouseGame, pours: rickhousePours || [], standings, activePour, myAnswer: rickhouseAnswer || null, myScore: (rickhouseScores || []).find((row: any) => row.player_id === currentPlayer?.id)?.score || 0 } : null;
